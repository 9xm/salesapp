...
class Site extends CI_Model
{
    ...
    public function getPIdata($from_quoteid)
    {
        $q =$this->db->get_where('quotes', ['id' => $from_quoteid], 1)->row();
        if(isset($q->pi_number)){
            return array(
    ...
    public function getUserByGroupName($group_name = 'employee')
    {

        $this->db->select('users.id as id, users.first_name as first_name, users.last_name as last_name')
            ->from('users')
