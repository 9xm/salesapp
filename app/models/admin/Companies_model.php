...
class Companies_model extends CI_Model
{
    ...
    public function addCustomerSite($data)
    {
        if ($this->db->insert('customer_site', $data)) {
            return true;
        }
        return false;
    }
    public function getCustomerSites($company_id)
    {
        $q = $this->db->get_where('customer_site', ['customer_id' => $company_id]);
        if ($q->num_rows() > 0) {
    ...
    public function updateCutomerSite($id, $data)
    {
        if ($this->db->update('customer_site', $data, ['id' => $id])) {
            return true;
        }
        return false;
    }
    ...
    {
        $q = $this->db->get_where('customer_site', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    }
    public function deleteCustomerSite($id)
    {
        if ($this->db->delete('customer_site', ['id' => $id])) {
            return true;
        }
        return false;
    }
    ...
    public function getCustomerSite($customer_id){
        
        //$q = $this->db->get_where('companies', ['id' => $customer_id], 1);
        $q = $this->db->get_where('customer_site', ['customer_id' => $customer_id]);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
