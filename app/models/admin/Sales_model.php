...
class Sales_model extends CI_Model
{
    ...
    public function addTask($sale_id,$data, $attachments){
        $this->db->update('sales', $data, ['id' => $sale_id]);

        $q = $this->db->get_where('attachments', ['subject_id' => $sale_id, 'subject_type' => 'saletask']);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $this->db->delete('attachments', ['id' => $row->id]);
            }
            //return $data;
        }
        if (!empty($attachments)) {
            foreach ($attachments as $attachment) {
                ...
                $attachment['subject_type'] = 'saletask';
