...
class Customers extends MY_Controller
{
    ...
    public function customer_site($company_id = null)
    {
        $this->sma->checkPermissions('index', true);
        $this->data['modal_js']  = $this->site->modal_js();
        $this->data['company']   = $this->companies_model->getCompanyByID($company_id);
        $this->data['customer_sites'] = $this->companies_model->getCustomerSites($company_id);
        $this->load->view($this->theme . 'customers/customer_site.php', $this->data);
    }

    public function add_customer_site($company_id = null)
    {
        $this->sma->checkPermissions('add', true);
        $company = $this->companies_model->getCompanyByID($company_id);

        $this->form_validation->set_rules('customer_site', 'Enter customer site', 'required');
        //$this->form_validation->set_rules('city', lang('city'), 'required');
        //$this->form_validation->set_rules('state', lang('state'), 'required');
        //$this->form_validation->set_rules('country', lang('country'), 'required');
        //$this->form_validation->set_rules('phone', lang('phone'), 'required');
        ...
        } elseif ($this->input->post('add_customer_site')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('customers');
        }

        if ($this->form_validation->run() == true && $this->companies_model->addCustomerSite($data)) {
            $this->session->set_flashdata('message', 'Customer site added successfully');
            admin_redirect('customers');
        } else {
            $this->data['error']    = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['company']  = $company;
            $this->load->view($this->theme . 'customers/add_customer_site', $this->data);
        }
    }
    public function edit_customer_site($id = null)
    {
        ...
        } elseif ($this->input->post('edit_customer_site')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('customers');
        }

        if ($this->form_validation->run() == true && $this->companies_model->updateCutomerSite($id, $data)) {
            $this->session->set_flashdata('message', 'Customer site update successfully');
            admin_redirect('customers');
        } else {
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['customer_site']  = $this->companies_model->getCustomerSiteByID($id);
            $this->load->view($this->theme . 'customers/edit_customer_site', $this->data);
        }
    }
    public function delete_customer_site($id)
    {
        ...
        if ($this->companies_model->deleteCustomerSite($id)) {
            $this->session->set_flashdata('message', 'Customer site deleted');
            admin_redirect('customers');
        }
    }
    public function customer_site_import_csv()
    {
        $this->sma->checkPermissions('add', true);
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES['csv_file'])) {
                ...
                if ($handle) {
                    //$arrResult=fgetcsv($handle);
                    while (($row = fgetcsv($handle, 5001, ',')) !== false) {
                        //$arrResult[] = implode(", ", $row);
                        if(!empty($row)){
                            $data = [
                                'customer_site'       => implode(", ", $row),
                                'customer_id'  => $this->input->post('customer_id'),
                            ];
                            $this->companies_model->addCustomerSite($data);
                        }
                    }
                    fclose($handle);
                }
    ...
    public function getCustomerSite(){
       ...
       $results = $this->companies_model->getCustomerSite($customer_id);
       $option='';
       if($results){
           foreach($results as $result){
                $option .="<option value='{$result->id}'>{$result->customer_site}</option>"; 
           }
           /*for($i=1; $i<7; $i++){
               $lbl=$result->customer_site;//{'cf'.$i};
               if(!empty($lbl)){
                $option .="<option value='cf{$i}'>{$lbl}</option>"; 
               }
           }*/
       }
       $this->sma->send_json(array('option'=> $option));
    }
