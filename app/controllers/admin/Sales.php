...
class Sales extends MY_Controller
{
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $customer_site      =$this->input->post('customer_site');
            //$from_quote=(isset($this->input->post('from_quote'))) ? $this->input->post('from_quote') : null; 
            //$po_number=(isset($this->input->post('po_number')))? $this->input->post('po_number'): null; 
            $from_quote=$this->input->post('from_quote'); 
            ...
            if(!empty($podate)){
                $podate=explode("/", $podate);
                $po_date           = "{$podate[2]}-{$podate[1]}-{$podate[0]}";//$this->sma->fld(trim($this->input->post('po_date')));//date('Y-m-d H:i:s', strtotime(trim($this->input->post('po_date'))));
            }
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment, [], $attachments)) {
            ...
            $this->data['employees']   = $this->site->getUserByGroupName('employee');
            $this->data['tax_rates']  = $this->site->getAllTaxRates();
            $this->data['units']      = $this->site->getAllBaseUnits();
            //$this->data['currencies'] = $this->sales_model->getAllCurrencies();
            $this->data['slnumber']    = ''; //$this->site->getReference('so');
            $this->data['payment_ref'] = ''; //$this->site->getReference('pay');
            $this->data['from_quote'] = $from_quote;

            if(!empty($from_quote)){
                $pidata= $this->site->getPIdata($from_quote);
                if($pidata){
                 $this->data['pi_number'] = $pidata['pi_number'];
                 $this->data['pi_date']= $pidata['pi_date'];
                }
             }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products, $attachments)) {
            ...
            if(!empty($inv->from_quote)){
                $pidata= $this->site->getPIdata($inv->from_quote);
                if($pidata){
                 $this->data['pi_number'] = $pidata['pi_number'];
                 $this->data['pi_date']= $pidata['pi_date'];
                }
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $task_addlink       = anchor('admin/sales/add_task/$1', '<i class="fa fa-file-text-o"></i> Add Task'); 

        if($this->Employee ){
            //$task_addlink       = anchor('admin/sales/add_task/$1', '<i class="fa fa-file-text-o"></i> Add Task'); 
            $detail_link       = anchor('admin/sales/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('sale_details')); 
            //$action = '<div class="text-center">' . $detail_link . ' </div>';
            $action = '<div class="text-center"><div class="btn-group text-left">'
            . '<button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'
            . lang('actions') . ' <span class="caret"></span></button>
            <ul class="dropdown-menu pull-right" role="menu">
                <li class="detailLink">' . $detail_link . '</li>
                <li class="addTaskLink">' . $task_addlink . '</li>
            </ul>
        </div></div>';
        }
        else{

           // $task_viewlink       = anchor('admin/sales/view_task/$1', '<i class="fa fa-file-text-o"></i> View Task'); 
        ...
        <ul class="dropdown-menu pull-right" role="menu">
            ...
            $action .= '<li  class="addTaskLink">' . $task_addlink . '</li>';
        ...
        if ($warehouse_id) {
            //echo "aaa";
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, {$this->db->dbprefix('sales')}.customer,  csite.customer_site, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
                ->join('customer_site as csite', " sales.customer_site=csite.id", 'left')
            ...
            $this->datatables
                ...
                ->from('sales')
                ->join('customer_site as csite', " sales.customer_site=csite.id", 'left');
              // ->where('sales.customer_site IS NULL ');
                ...
                //->union('sales')
                //->select("sales.id as id, DATE_FORMAT(sales.date, '%Y-%m-%d %T') as date, reference_no, biller, sales.customer, sales.customer_site, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, sales.attachment, return_id")
                //->from('sales')
                //->where('sales.customer_site IS NULL');

                // OR sales.customer_site IS NULL
               // if($this->Employee ){
                    //echo "bbbb====".$this->session->userdata('user_id');
                 //   $this->datatables->where('sales.employee_id', $this->session->userdata('user_id'));
                //}                
        }


        if($this->Employee ){
            $this->datatables->where('employee_id',  $this->session->userdata('user_id'));
        }
        else{     
    ...
    public function add_task($id = null)
    {
        ...
        if (!empty($_POST['latitude'])) {
            ...
            $attachments        = $this->attachments->upload();
            $data['taskattachment'] = !empty($attachments); 
            $data['task_datetime'] =date("Y-m-d H:i"); 
            //print_r($attachments);
            //die;
            $this->sales_model->addTask($sale_id,$data, $attachments);
            $this->session->set_flashdata('message', lang('Task added successfully.'));
            admin_redirect('sales');
        ...
        $this->page_construct('sales/add_task', $meta, $this->data);
    }
    ...
    public function pdf($id = null, $view = null, $save_bufffer = null)
    {
        ...
        $customer_site_2=$this->sales_model->getCustomerSiteByID($inv->customer_site);
        $this->data['customer_site_2'] =$customer_site_2->customer_site;
        ...
        $html = $this->load->view($this->theme . 'sales/pdf2', $this->data, true);
        if (!$this->Settings->barcode_img) {
            $html = preg_replace("'\<\?xml(.*)\?\>'", '', $html);
        }

        $this->data['biller']->invoice_footer='';
        //$this->data['biller']->invoice_footer='<div style="float:right">Original for Recipient</div>';
        //print_r($this->data['biller']->invoice_footer);
        //die;
        //$view=true;

        if ($view) {
            $this->load->view($this->theme . 'sales/pdf2', $this->data);
        } elseif ($save_bufffer) {
    ...
    public function view($id = null)
    {
        ...
        $this->data['skrill']      = $this->sales_model->getSkrillSettings();
        $this->data['attachments'] = $this->site->getAttachments($id, 'sale');
        $this->data['taskattachments'] = $this->site->getAttachments($id, 'saletask');
       

        //getCustomerSiteByID();
        
        $employee= $this->site->getUser($inv->employee_id);
       ...
       /* echo "<pre>";
        print_r($this->site->getUser($inv->employee_id));
        print_r($this->data['inv'] );
        echo "</pre>";*/
