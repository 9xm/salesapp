...
class Reports extends MY_Controller
{
    ...
    public function getPurchasesReport2($pdf = null, $xls = null)
    {
        $this->sma->checkPermissions('purchases', true);
        ...
        if ($pdf || $xls) {
           /* $this->db
            ->select('' . $this->db->dbprefix('purchases') . '.cgst as cgst,' . $this->db->dbprefix('purchases') . '.igst as igst,' . $this->db->dbprefix('purchases') . '.sgst as sgst,' . $this->db->dbprefix('products') . '.cost as cost,' . $this->db->dbprefix('purchases') . '.date, reference_no, ' . $this->db->dbprefix('warehouses') . '.name as wname,  ' . $this->db->dbprefix('products') . '.code as product_code, ' . $this->db->dbprefix('products') . '.hsn_code as hsn_code, supplier, ' . $this->db->dbprefix('purchase_items') . ".product_name as product_name,  " . $this->db->dbprefix('purchase_items') . ".quantity as qty, grand_total, paid, " . $this->db->dbprefix('purchases') . '.status', false)
            ->from('purchases')
            ->join('purchase_items', 'purchase_items.purchase_id=purchases.id', 'left')
            ->join('warehouses', 'warehouses.id=purchases.warehouse_id', 'left')
            ->join('products', 'products.id=purchase_items.product_id', 'left')
            ...
            ->order_by('purchases.date desc');*/
            $this->db
                ->select(''. $this->db->dbprefix('purchases') . '.id as purchase_id, ' . $this->db->dbprefix('purchases') . '.date, reference_no, ' . $this->db->dbprefix('warehouses') . '.name as wname, supplier, grand_total, paid, ' . $this->db->dbprefix('purchases') . '.status', false)
                ->from('purchases')
                ->join('purchase_items', 'purchase_items.purchase_id=purchases.id', 'left')
                ->join('warehouses', 'warehouses.id=purchases.warehouse_id', 'left')
                //->join('products', 'products.id=purchase_items.product_id', 'left')
            ...
            if ($q->num_rows() > 0) {
                ...
                if (!empty($data)) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('C1', 'PART NO');
                    $this->excel->getActiveSheet()->SetCellValue('D1', 'PART NAME');
                    $this->excel->getActiveSheet()->SetCellValue('E1', 'QUANTITY');
                    $this->excel->getActiveSheet()->SetCellValue('F1', 'RATE');
                    $this->excel->getActiveSheet()->SetCellValue('G1', 'BASIC VALE');
                    $this->excel->getActiveSheet()->SetCellValue('H1', 'GODOWN');
                    $this->excel->getActiveSheet()->SetCellValue('I1', 'HSNCODE');
                    $this->excel->getActiveSheet()->SetCellValue('J1', 'TEXABLE VALUE');
                    $this->excel->getActiveSheet()->SetCellValue('K1', 'IGST');
                    $this->excel->getActiveSheet()->SetCellValue('L1', 'SGST');
                    $this->excel->getActiveSheet()->SetCellValue('M1', 'CGST');
                    $this->excel->getActiveSheet()->SetCellValue('N1', 'TOTAL VALUE');
                    
                    //$this->excel->getActiveSheet()->SetCellValue('O1', lang('reference_no'));
                    //$this->excel->getActiveSheet()->SetCellValue('C1', lang('warehouse'));
                    //$this->excel->getActiveSheet()->SetCellValue('D1', lang('supplier'));
                    //$this->excel->getActiveSheet()->SetCellValue('E1', lang('product_qty'));
                    //$this->excel->getActiveSheet()->SetCellValue('F1', lang('grand_total'));
                    ...
                    foreach ($data as $data_row) {
                        ...
                        $this->db
                        ->select('' . $this->db->dbprefix('purchase_items') . '.cgst as cgst,' . $this->db->dbprefix('purchase_items') . '.igst as igst,' . 
                        $this->db->dbprefix('purchase_items') . '.sgst as sgst,' . 
                        $this->db->dbprefix('purchase_items') . '.net_unit_cost as cost,'.
                         $this->db->dbprefix('purchase_items') . '.quantity as qty,' . 
                         //$this->db->dbprefix('purchases') . '.id as purchase_id, ' . 
                         //$this->db->dbprefix('purchases') . '.date, reference_no, ' . 
                         //$this->db->dbprefix('warehouses') . '.name as wname,'. 
                         $this->db->dbprefix('purchase_items') . '.product_code as product_code, ' . $this->db->dbprefix('products') . '.hsn_code as hsn_code, '. 
                         $this->db->dbprefix('purchase_items') . ".product_name as product_name,  " , false) //$this->db->dbprefix('purchases') . '.status', false)
                        ->from('purchase_items')
                        //->join('purchase_items', 'purchase_items.purchase_id='.$data_row->purchase_id, 'left')
                        //->join('warehouses', 'warehouses.id=purchases.warehouse_id', 'left')
                        ->join('products', 'products.id=purchase_items.product_id', 'left');
                        //->group_by('purchases.id')
                        //->order_by('purchases.date desc');
                        $this->db->where('purchase_items.purchase_id', $data_row->purchase_id);
                        ...
                        //print_r($data_row);
                        //die;
                        $product_cost=$data2_row->cost;
                        $product_qty=$data2_row->qty;

                        $product_price=$product_cost*$product_qty;

                        $product_total_val=$product_price+$data2_row->igst+$data2_row->sgst+$data2_row->cgst;
                        //echo $slno;

                        //var_dump($row);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row1, $slno);

                        //$this->excel->getActiveSheet()->SetCellValue('B' . $row, $this->sma->hrld($data_row->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row1, $data_row->date);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row1, $data2_row->product_code);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row1, $data2_row->product_name);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row1, $product_qty);
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row1, $product_cost);
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row1, $product_price);
                        $this->excel->getActiveSheet()->SetCellValue('H' . $row1, $data_row->wname);
                        $this->excel->getActiveSheet()->SetCellValue('I' . $row1, $data2_row->hsn_code);
                        $this->excel->getActiveSheet()->SetCellValue('J' . $row1, $product_price);
                        $this->excel->getActiveSheet()->SetCellValue('K' . $row1, $data2_row->igst);
                        $this->excel->getActiveSheet()->SetCellValue('L' . $row1, $data2_row->sgst);
                        $this->excel->getActiveSheet()->SetCellValue('M' . $row1, $data2_row->cgst);
                        $this->excel->getActiveSheet()->SetCellValue('N' . $row1, $product_total_val);
                        //$this->excel->getActiveSheet()->SetCellValue('O' . $row1, $data_row->reference_no);
