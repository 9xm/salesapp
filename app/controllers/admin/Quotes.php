...
class Quotes extends MY_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $pidate=$this->input->post('pi_date');
            $pi_date =null;
            if(!empty($pidate)){
                $pidate=explode("/", $pidate);
                $pi_date           = "{$pidate[2]}-{$pidate[1]}-{$pidate[0]}";//$this->sma->fld(trim($this->input->post('po_date')));//date('Y-m-d H:i:s', strtotime(trim($this->input->post('po_date'))));
            }
            //$pi_date           = date('Y-m-d');
            //print_r(array($po_date, $pi_date));
            //die;
